...
test('@test Locators Test', async ({ page }) =>{ 
    ...
    element check chesi vunte true ani lekkapothe false return chesthundi, aithe isChecked() 
    method not used for assertion
    */
    ...
    */
    ...
    /*
    adena element nee check(click() method) chesinatrvata adi checked or not anedi toBeChecked() 
    method not assertion chesi check cheyochu
    but diselect(uncheck()) chesina vattiki assesstion apply cheyalem anduku ante ikkada for selected 
    options ki toBeCheckd() ane assesstion vundi kani unchecked ki ledt
    */
    expect(await agreecheckbox.isChecked()).toBeFalsy();
    /*
    aithe assertions cheyalemu kani a elemnt yakka state my expected behaviour lo match avuthunda 
    ledaa check cheyochu
